...
load(paste0("results/", ifelse(test = igear == "otter",
                               yes = "otter_trawl",
                               no = "beam_trawl_2012_2019"),
            "/survey_opt - ", istrata, " STRATA", "/MS/Run_", 
            n_runs, "/result_list.RData"))
...
for (itarget in target_n){ ## Loop over sampling effort -- start
  ...
  for (iter in 1:500) { ## Loop over iterations -- start
    ...
    sample_df <- ms_dens[, sample_vec, 1]
...
{
  png(filename = "presentations/results_12_07_2021/sim_4_surveys.png",
  ...
  for (ispp in 1:n_spp) {
    ...
    plot(x = target_n, y = true_cv_ms_strs[, ispp], 
         las = 1, pch = "*", col = "red", axes = F, ann = F,
         ylim = c(0, ymax),  xlim = c(35, 105))
    box()
    lines(x = target_n, y = true_cv_ms_strs[, ispp],)
    
    boxplot(t(cv_ms_strs[, ispp, ]), add = TRUE,
            at = target_n, names = target_n, 
            pch = 16, cex = 0.25, pars = list(boxwex = 3), las = 1,
            ylim = c(0, max(cv_ms_strs[, ispp, ])),  xlim = c(35, 105), axes = F)
    points(x = target_n, 
           y = true_cv_ms_strs[, ispp], pch = "*", cex = 3, col = "red")
    if (ispp == 1) mtext(side = 3, text = "MS STRS",  line = 0.5, cex = 1.25)
    if(ispp == n_spp) axis(side = 1)
...
{
  png(filename = "presentations/results_12_07_2021/sim_4_surveys_RRMSE_CV.png", 
      width = 10, height = 10, units = "in", res = 500)
  par(mar = c(0.25, 0, 0.25, 0), mfrow = c(n_spp, 3), oma = c(4, 5, 3, 10))
  for (ispp in 1:n_spp) {
    ymax <- max(c(rrmse_cv_srs[, ispp], rrmse_cv_sys[, ispp], 
                  rrmse_cv_ms_strs[, ispp] )) * 1.1
    ...
    plot(x = target_n, y = rrmse_cv_ms_strs[, ispp], axes = F, 
         xlim = c(35, 105), ylim = c(0, ymax),
         pch = 16); box()
    lines(x = target_n, y = rrmse_cv_ms_strs[, ispp])
