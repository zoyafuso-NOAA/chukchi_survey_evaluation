...
index_sys <- apply(X = index_sys, 
                   MARGIN = 1:2, 
                   FUN = function(x) ifelse(test = x > mean(x) + 3*sd(x), 
                                            yes = NA, 
                                            no = x ))

true_cv_sys <- sweep(x = apply(X = index_sys, 
                               MARGIN = 2:3, 
                               FUN = sd, 
                               na.rm = TRUE), 
                     MARGIN = 2, 
...
rrmse_cv_sys <- sqrt(apply(X = sweep(x = cv_sys,
  ...
  apply(X = cv_sys, MARGIN = 2:3, FUN = mean, na.rm = TRUE) 
...
save(list = c("index_sys", "cv_sys", "rb_index_sys", "true_cv_sys", 
     ...
     file = paste0("results/chukchi_", igear, "/sys_L05_survey_sim_results.RData"))
