...
for (istrata in stratas[1]) {
  ...
  ##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  ss_sample_allocations <- expand.grid(n = total_n, species = spp_list$taxon)
  for (ispp in 1:n_spp) {
    ...
    for (isample in total_n){
      ...
      temp_idx <- ss_sample_allocations$n == isample & 
        ss_sample_allocations$species == spp_list$taxon[ispp]
  ...
  plot_survey_opt_map(file_name = paste0("solution_nbs.png"),
                      ...
                      allocations = as.numeric(
                        ms_sample_allocations[ms_sample_allocations$n == 190, 
                                              paste0("Str_", 
                                                     1:length(temp_bethel))]),
                      draw_stations = TRUE)
