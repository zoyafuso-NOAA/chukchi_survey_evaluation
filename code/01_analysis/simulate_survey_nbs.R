...
grid_pts <- readOGR(here::here("data/survey_opt_data/grid_pts_nbs.shp"))

##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##   Create an outline of the NBS grid in aea projection. This mask is used
##       when creating the systematic grids. 
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
nbs_mask <-
  rgdal::readOGR(here::here("data/spatial_data/survey_boundary/NBS_2019.shp"))
...
ms_dens <- array(dim = c(n_spp, n_cells, n_iters), 
                 dimnames = list(spp_list$taxon, NULL))
...
index_srs <- cv_srs <- rb_index_srs <-
  array(dim = c(length(target_n), n_spp, n_iters),
        dimnames = list(paste0("srs_n = ", target_n), spp_list$taxon, NULL))
...
while (temp_res >= 35000) {
  ...
  temp_grid <- sp::makegrid(x = grid_pts, 
                            cellsize = temp_res, 
                            pretty = F)
  temp_grid <- sp::SpatialPoints(coords = coordinates(temp_grid))
  temp_grid <- terra::intersect(x = temp_grid, y = nbs_mask)
  ...
  grid_idx <- apply(X = rgeos::gDistance(spgeom1 = grid_pts,
                                         spgeom2 = temp_grid,
                                         byid = TRUE),
                    MARGIN = 1,
                    FUN = which.min)
  ...
  distances <- as.matrix(dist(grid_pts[grid_idx, ]@coords))
  ...
  neighbor_var <- matrix(nrow = length(min_distances), ncol = n_spp,
                         dimnames = list(NULL, spp_list$taxon))
...
rm(sys_sample, sys_mean, sys_sd_LO5, sys_index, sys_sd, 
   ...
   temp_index_sys, temp_cv_sys, temp_cv_sys_LO5)
...
index_ms_strs <- cv_ms_strs <- rb_index_ms_strs <-
  array(dim = c(length(target_n), n_spp, n_iters),
        dimnames = list(paste0("ms_strs_n = ", target_n), spp_list$taxon, NULL))
...
for (itarget in target_n){ ## Loop over sampling effort -- start
  ...
  allocation <- as.integer(subset(ms_sample_allocations, 
                                  n == itarget)[, paste0("Str_", 1:n_strata_realized)])
...
##   Save results ----
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
save(file = here::here(paste0("results/nbs_", igear, 
