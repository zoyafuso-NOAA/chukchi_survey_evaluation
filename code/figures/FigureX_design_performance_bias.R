...
for (taxon in c("fish", "inverts")) {
  ...
  for (ispp in 1:length(idx)) { ## Loop over species -- start
    ...
    ylim = max(c(max(abs(rowMeans(rb_index_srs[, idx[ispp], ])),
               abs(rowMeans(rb_index_ms_strs[, idx[ispp], ])),
               abs(rowMeans(rb_index_sys[, idx[ispp], ]))) * 1.1,
               25))
    
    plot(1, type = "n", axes = F, ann = F, 
         ylim = c(-ylim, ylim), xlim = c(35, 180))
    
    points(x = target_n, y = rowMeans(rb_index_srs[, idx[ispp], ]), pch = 16)
    lines(x = target_n, y = rowMeans(rb_index_srs[, idx[ispp], ]), pch = 16)
    
    
    points(x = sys_settings$n, y = rowMeans(rb_index_sys[, idx[ispp], ]), 
           pch = 16, col = "blue")
    lines(x = sys_settings$n, y = rowMeans(rb_index_sys[, idx[ispp], ]), 
          pch = 16, col = "blue")
    
    points(x = target_n, y = rowMeans(rb_index_ms_strs[, idx[ispp], ]), 
           pch = 16, col = "orange")
    lines(x = target_n, y = rowMeans(rb_index_ms_strs[, idx[ispp], ]), 
          pch = 16, col = "orange")
    ...
    ylim = max(c(max(abs(rowMeans(rb_cv_srs[, idx[ispp], ])),
               abs(rowMeans(rb_cv_ms_strs[, idx[ispp], ])),
               abs(rowMeans(rb_cv_sys[, idx[ispp], ]))) * 1.1,
               25))
    
    plot(1, type = "n", axes = F, ann = F, 
         ylim = c(-ylim, ylim), xlim = c(35, 180))
    
    points(x = target_n, y = rowMeans(rb_cv_srs[, idx[ispp], ]), pch = 16)
    lines(x = target_n, y = rowMeans(rb_cv_srs[, idx[ispp], ]), pch = 16)
    
    
    points(x = sys_settings$n, y = rowMeans(rb_cv_sys[, idx[ispp], ]), 
           pch = 16, col = "blue")
    lines(x = sys_settings$n, y = rowMeans(rb_cv_sys[, idx[ispp], ]), 
          pch = 16, col = "blue")
    
    points(x = target_n, y = rowMeans(rb_cv_ms_strs[, idx[ispp], ]), 
           pch = 16, col = "orange")
    lines(x = target_n, y = rowMeans(rb_cv_ms_strs[, idx[ispp], ]), 
          pch = 16, col = "orange")
  ...
  ## Axis Labels
  mtext(side = 2, text = "% Bias of Index", outer = TRUE, line = 1, font = 2)
